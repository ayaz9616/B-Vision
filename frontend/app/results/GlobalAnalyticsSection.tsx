...
export default function GlobalAnalyticsSection({ data, selectedPhoneNames }: GlobalAnalyticsSectionProps) {
  ...
  return (
    <div className="w-full mb-8 mt-12">
      <h3 className="text-2xl font-bold text-blue-900 mb-4">📈 Global Analytics</h3>
      {/* 1. Age group and feature preference */}
      {false && (
      <div className="mb-4">
        <span className="font-semibold">Which age group likes which feature the most:</span>
        <ul className="list-disc ml-6 mt-1">
          ...
          )) : <li>No age-feature data available for the selected phones.</li>}
        </ul>
      </div>
      )}
      {/* 2. Brand performance */}
      <div className="mb-4">
        <span className="font-semibold">Brands performance (decreasing order):</span>
        <ul className="list-disc ml-6 mt-1">
          {brandPerf.length ? brandPerf.map(({ brand, positive, products }) => (
            <li key={brand} className="mb-2">
              <div>
                <span className="text-blue-700 font-semibold">{brand}</span>: {positive} positive reviews
              </div>
              <div className="ml-4 text-sm text-gray-600">
                Models: {products.join(", ")}
              </div>
            </li>
          )) : <li>No brand data available.</li>}
        </ul>
      </div>
      {/* 3. Model performance */}
      <div className="mb-4">
        <span className="font-semibold">Models performance (decreasing order):</span>
        <ul className="list-disc ml-6 mt-1">
          {modelPerf.length ? modelPerf.map(({ name, positive }) => (
            <li key={name} className="mb-1">
              <span className="text-blue-700 font-semibold">{name}</span>: {positive} positive reviews
            </li>
